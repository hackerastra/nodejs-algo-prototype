...
exports.f1_nbController = async function(req,res,next){
    let recipientEmailAddress = await assignGC.assignedGC();
    let dist = await distance.distance() 
    var mailOptions = {
      from: config["email-id"],
      to: recipientEmailAddress,
      subject: 'Peak Value for Floor 1 - Building XYZ , Landmark ABC, City : ### - Non Biodegradable Dustbin Reached',
      text: 'Please Reach Out To Apartment, XYZ - Floor 1 For Collection Of Wastage @ c/o Non Biodegradable Dustbin.'+ 'Distance From Your Location To Appartment Is: ' + dist + 'km.' 
    };
